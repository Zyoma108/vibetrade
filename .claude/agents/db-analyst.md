...
порче WAL-БД, см. `docs/database.md`). Значит **хостовый `data/trading_bot.db` не
...
`strategy`/`trading`) и `docs/strategy.md` + `docs/positions.md`.** Это живой, поддерживаемый журнал экспериментов (даты, цифры,
...
и историю экспериментов в `config.yaml` и `docs/` перед Фазой 4** (см. выше), этот раздел
...
### Отвергнутые/завершённые идеи — краткий указатель (актуальный источник: `config.yaml` + `docs/decisions.md`)
...
| `AGENTS.md` | Ядро документации: жёсткие правила (в т.ч. «К базе — только через `docker exec`») + карта `docs/` |
| `docs/strategy.md`, `docs/positions.md` | Детектор и фильтры / позиции и Circuit Breaker + история экспериментов |
| `docs/backtest.md` | ⚠️ Правила parity — читать перед любой правкой движка бэктеста или свипов |
...
11. **Изучи актуальный код и конфиг** (см. «Текущая архитектура», но перепроверяй по `config.yaml` и `docs/`) прежде чем предлагать изменения — многие идеи уже реализованы или отвергнуты, и этот раздел может отставать.
...
6. **Фаза 4 (рекомендации)** — конкретные изменения с именами файлов и параметров, после сверки с `config.yaml` и `docs/`.

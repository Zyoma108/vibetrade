...
**Оцени:** нужно ли было вообще открывать позицию в этом режиме? Если regime=risk_off или cautious с падающим BTC — **сигнал должен был быть отфильтрован.** Запиши это.
...
- `partial_closed=0` на убыточной сделке — частичное закрытие не сработало? Почему?
...
7. **Сравнивай с бэктестом.** Бэктест — это upper bound того, что стратегия МОЖЕТ дать. Если реальность сильно хуже — ищи операционные потери, а не стратегические.
...
2. **Фаза 1 (конвейер)** — 2-3 минуты. Сразу видно operational проблемы (error, duplicate).

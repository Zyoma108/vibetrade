...
### Размер: `ix_open_interest_timestamp` остался без читателей (23.09.2026)
...
OI — 57.6% базы (таблица плюс два индекса). При этом `ix_open_interest_timestamp` держался
на удалении по ретенции, а `retention_days` удалён 23.09.2026 (вся история нужна для
бэктестов, см. `AGENTS.md`, правило 4). Единственный оставшийся читатель — `SELECT
MAX(timestamp)` в загрузчике бэктеста, и только при `limit_days`; без индекса это один
полный проход на загрузку, не в цикле бота. То есть **13.5% размера и часть времени каждой
вставки уходят ни на что.** Снятие — в ручной список `DROP INDEX` в `init_db()`, туда же,
где сняты `ix_open_interest_exchange` и `ix_open_interest_symbol`. НЕ сделано: отдельная
правка, не смешивается с окном OI-гейта.

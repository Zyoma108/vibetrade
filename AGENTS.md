...
Регрессия закрыта тестами `test_oi_filter_disabled_lets_signals_through` и
`TestOiTrendPasses`. **Проверка перед любым изменением движка:** прогнать бэктест на
одной и той же БД до и после и сравнить список сделок — сравнение двух прогонов ПОСЛЕ
изменения ничего не доказывает.

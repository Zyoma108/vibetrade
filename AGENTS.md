...
- **SQLAlchemy 2.0 + aiosqlite** — SQLite в DELETE-режиме (не WAL — см. "База данных")
...
Бот работает в Docker-контейнере (`docker-compose.yml`, `container_name: trading-bot`),
`data/` смонтирована с хоста как bind mount. Оркестратор и сабагенты живут на хосте (там же, где
залогинен Claude Code) — если вызвать `scripts/agent_*.py` или `sqlite3 data/trading_bot.db`
напрямую с хоста, ПАРАЛЛЕЛЬНО с работающим ботом внутри контейнера, блокировки SQLite не
гарантированно согласуются через границу Docker Desktop. 21.07.2026 это уже привело к порче
`market_context_snapshots` ("database disk image is malformed") во время ручного тестирования
скриптов с хоста. **Все обращения к БД — только `docker exec trading-bot <команда>`** (Dockerfile
копирует `scripts/` в образ специально для этого). Скилл и оба сабагента уже написаны с этим
требованием — не убирать обёртку при правке.
...
SQLite в **DELETE-режиме** (`data/trading_bot.db`), не WAL. Миграции: Alembic + ручной `ALTER TABLE` в `init_db()` для обратной совместимости.

**Почему не WAL**: `data/` — bind-mount том Docker Desktop for Mac (`docker-compose.yml`,
`./data:/app/data`). WAL полагается на shared-memory индекс (`-shm`) через `mmap` для координации
между соединениями — это ненадёжно на bind-mount через osxfs/gRPC-FUSE, независимо от того, кто
пишет: хост или несколько соединений внутри одного контейнера. 21-22.07.2026 это дважды привело к
порче БД (`market_context_snapshots`, затем индекс `ix_candles_symbol`) — второй раз, судя по
всему, из-за увеличившегося числа конкурентных SQLite-соединений внутри контейнера после
включения `agent.enabled` (`_agent_watch_loop`/`_agent_position_loop` — отдельные сессии поверх
основного цикла сборщика). `_set_journal_mode()` в `database.py` переводит БД на `DELETE`
(обычные файловые локи вместо mmap) при каждом подключении — медленнее при конкурентной записи,
надёжнее на этой файловой системе. Если повреждение повторится и на DELETE — следующий шаг:
перевести `data/` на named Docker volume (не bind-mount).

...
| `max_window_retracement_pct` | 2.0 (прод, live-эксперимент с 10.08.2026) / 0.0 (схема) | Макс. откат от пика sustain-окна до последнего close, % (0 = выкл) |
...
**Retracement filter — статус эксперимента (10.08.2026):** свип `scripts/sweep_retracement.py` на `data/trading_bot.db` (24.07–09.08, n=47 сделок) дал th=2.0 как лучший порог (WR 66.7% / PnL +$87.62 против бейзлайна 63.8% / $52.68 при th=0), но статистика слабая — корреляция retracement↔PnL незначима (r=-0.13, немонотонные бакеты), а th=1.0 на том же прогоне оказался явно вредным (отсекает больше хороших сделок, чем плохих). Включено в проде на свой страх и риск как live-эксперимент, а не подтверждённая оптимизация. **Пересмотреть через 2-3 недели живой торговли** — сравнить фактический win rate/PnL с бейзлайном из db-audit-august-2026 (47.9% WR, -$1.80 за 16 дней); если улучшения нет — откатить `max_window_retracement_pct` на `0.0`. Отдельно стоит попробовать расширить окно расчёта отката с одного sustain-окна (4 свечи/12 мин) до sustain+pre-surge (~14 свечей/42 мин) — точнее соответствует паттерну из живого аудита — и пересвипнуть.

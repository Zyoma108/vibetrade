...
### Partial close — проверка по циклу (virtual) vs ордерами (real)

Текущая реализация проверяет частичное закрытие в `update_positions()` по `current_price` из тикера раз в `interval_seconds`. Это означает, что кратковременные ценовые пики могут быть пропущены.

**Решение (real):** при открытии позиции выставлять лимитный ордер на частичную фиксацию + conditional stop в безубыток. Убирает зависимость от цикла опроса.

**Решение (virtual):** проверять закрытие по high/low свечей, а не по тикеру (как бэктест). Данные свечей уже есть в БД.

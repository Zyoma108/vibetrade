...
│   ├── test_position_manager.py  # Тесты PositionManager (Circuit Breaker, TP/SL, fallback партиала, цена входа, б/у-ретрай, детект партиал-филла)
│   ├── test_market_data_collector.py # Тесты MarketDataCollector (дозревание свечи, снимок tickers, батч коммитов, дедлайн фазы, ретенция)
│   └── test_exchange_connector.py    # Тесты ExchangeConnector (политика scan vs торговля, отсутствующие монеты)
config/
...
|---|---|---|
| Сеть: 580 × 0.33с ÷ слоты семафора | ~38 с при 5 слотах | нет, но зависит от «погоды» |
...
каких барах движок ищет сигналы. Дефолт 105 с не соответствовал реальности ни
одного дня из четырёх (факт: 130 с в лучшие дни, 400-1600 с в худшие). После

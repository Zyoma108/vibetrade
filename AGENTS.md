...
Узкое место — РАБОТА цикла, а не пауза: 23.09.2026 измерено, что каданс деградировал за
сентябрь 65 → 118 с вместе с ростом БД и полностью восстановился до 68 с на пересозданной
базе. Кластер 16.09, давший 55% разрыва с бэктестом, потерян в разрыве 190 с, из которых
пауза — 45 с. Поэтому `interval_seconds` снижать бесполезно; чистки истории нет
(`retention_days` удалён 23.09.2026 — вся история нужна для бэктестов), рычаги —
настройки SQLite (сделано), объём записи OI и апгрейд VPS.
Следить не за медианой, а за сканами на бар (сейчас 2.6) и долей циклов длиннее бара
(сейчас 0.1%). Подробности и разбор бюджета цикла: `docs/operations.md`.
Тот же механизм молча переопределил окно OI-гейта: «3 последние записи OI» — это ДВА
КАДАНСА, то есть 9.3 мин в августе и 2.3 мин сейчас, при неизменной настройке. Исправлено
временным окном 23.09.2026 (`oi_trend_window_bars`), вердикт по гейту не изменился —
`docs/strategy.md`. Мораль общая: любая величина, заданная ЧИСЛОМ ЗАПИСЕЙ, а не временем,
— это скрытая функция от каданса.
...
Порог доли тейков задан ДЛЯ RR 2.0 И ДЛЯ ЧАСТОТЫ 4.1 сделки в сутки. Оба условия
обязательны: 23.09.2026 свип `max_window_range_pct` дал конфигурацию с долей тейков
25.6% (выше цели) и при этом МЕНЬШИМ R/сут, потому что частота упала с 4.3 до 3.3.
Поэтому любое изменение, которое двигает число сделок, судится по R/сут, а доля тейков
читается только рядом с ней. При смене `risk_reward_ratio` пороги пересчитывать.

...
2. **Бюджет риска**: `баланс × risk_per_trade_pct / 100` (real — с биржи, virtual — $1000)
3. **Размер позиции**: `quantity = risk_budget / ATR` — чем волатильнее монета, тем меньше позиция
4. **TP/SL**: `sl = entry - ATR`, `tp = entry + ATR × risk_reward_ratio`
...
- **Стоп-лосс**: всегда `SL = 1 × ATR` (одна единица волатильности)
- **Тейк-профит**: `TP = ATR × risk_reward_ratio` (например, 3 ATR при ratio=3.0)
- **Размер позиции**: `quantity = risk_budget / ATR`, где `risk_budget = баланс × risk_per_trade_pct / 100`
  - Для волатильных монет (высокий ATR) позиция автоматически меньше — риск в долларах фиксирован
  - Virtual режим: баланс = $1000 (фиксированный)
- **Источник**: последние `atr_period + 1` свечей из БД (таймфрейм коллектора, обычно 3m)
- **True Range**: `max(high - low, |high - prev_close|, |low - prev_close|)`
- **ATR**: среднее арифметическое TR за `atr_period` баров (без Wilder-сглаживания)
- **Fallback**: если свечей недостаточно → SL = 5% от цены входа
...
- **ATR fallback** — если у монеты недостаточно свечей в БД за 24 часа (< period+1), стоп устанавливается на 5% от цены входа. Это может случиться для только что залистившихся монет.

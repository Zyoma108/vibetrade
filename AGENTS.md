...
**`database is locked` на сохранении состояния (фикс 12.08.2026):** `_save_state()` пишет через
свой ОТДЕЛЬНЫЙ `async_session()` из fire-and-forget задачи (`_schedule_save_state` →
`asyncio.create_task`), а `Application._on_collect_cycle_done` держит СВОЙ `session` открытым на
весь цикл сборщика и коммитит один раз в самом конце (см. "Нюансы" ниже — полный скан рынка может
занимать несколько минут). Если мутация Circuit Breaker (например, сброс серии убытков прибыльной
сделкой) происходит в середине такого цикла, независимая запись `_save_state()` упирается в занятый
SQLite-writer и падает с `OperationalError: database is locked` после 30-секундного
`connect_args={"timeout": 30}` — именно это увидели в рантайм-логе 12.08.2026 (ровно 30с между
логом сброса серии и ошибкой). Не критично (штатные TP/SL на бирже — основная защита независимо от
исхода этой записи, а следующая успешная мутация просто перезапишет актуальное состояние), но
означает, что персист не гарантирован под нагрузкой. **Фикс:** `_save_state()` теперь ретраится на
`OperationalError` с бэкоффом (10/20/40/80с, ~150с общий бюджет — дёшево, поскольку фоновая
задача никого не блокирует). Корневая причина (одна гигантская транзакция на весь цикл в
`_on_collect_cycle_done`) НЕ устранена — более глубокий фикс (коммитить чаще внутри цикла) заведомо
рискованнее на живой торговле и отложен.

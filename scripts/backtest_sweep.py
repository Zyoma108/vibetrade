...
DB_PATH = "data/trading_bot_29.05-07.06.db"
...
async def run_one(label: str, overrides: dict) -> dict | None:
    """Run a single backtest with given overrides."""
    ...
    try:
        ...
        if result and result["trades"] > 0:
            return {
                ...
                "profit_factor": round(
                    sum(1 for t in result["trades_list"] if t.pnl > 0) / max(
                        sum(1 for t in result["trades_list"] if t.pnl <= 0), 1
                    ) * 2,  # approximate
                    2
                ) if result["trades_list"] else 0,
            }
...
async def main():
    ...
    # =========================================================================
    print("\n" + "=" * 80)
    print("  BACKTEST SWEEP — trading_bot_29.05-07.06.db")
    print("=" * 80)

    print("\n[1/9] Baseline (current config)...")
    baseline = await run_one("BASELINE (RR=3.0 SL=5% vol=15 cool=1h)", {})
    if baseline:
        all_results.append(baseline)
        print(f"  → {baseline['trades']} trades, {baseline['win_rate']}% WR, PnL=${baseline['total_pnl']:+.2f}")

    # =========================================================================
    # ROUND 1: risk_reward_ratio
    # =========================================================================
    print("\n[2/9] Sweeping risk_reward_ratio...")
    rr_results = []
    for rr in [2.0, 2.5, 3.0, 3.5, 4.0, 5.0]:
        r = await run_one(f"RR={rr:.1f}", {"risk_reward_ratio": rr})
        if r:
            rr_results.append(r)
            print(f"  → RR={rr:.1f}: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, TP={r['tp']}, SL={r['sl']}")
    all_results.extend(rr_results)

    # =========================================================================
    # ROUND 2: volume_surge_mult
    # =========================================================================
    print("\n[3/9] Sweeping volume_surge_mult...")
    vol_results = []
    for vol in [10.0, 12.0, 15.0, 18.0, 20.0, 25.0]:
        r = await run_one(f"vol_surge=x{vol:.0f}", {"volume_surge_mult": vol})
        if r:
            vol_results.append(r)
            print(f"  → vol=x{vol:.0f}: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, Sig={r['signals']}")
    all_results.extend(vol_results)

    # =========================================================================
    # ROUND 3: cooldown_hours
    # =========================================================================
    print("\n[4/9] Sweeping cooldown_hours...")
    cool_results = []
    for ch in [0.0, 0.5, 1.0, 2.0, 4.0, 8.0]:
        r = await run_one(f"cooldown={ch}h", {"cooldown_hours": ch})
        if r:
            cool_results.append(r)
            print(f"  → cool={ch}h: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}")
    all_results.extend(cool_results)

    # =========================================================================
    # ROUND 4: stop_loss_pct
    # =========================================================================
    print("\n[5/9] Sweeping stop_loss_pct...")
    sl_results = []
    for slp in [3.0, 4.0, 5.0, 6.0, 7.5, 10.0]:
        r = await run_one(f"stop_loss={slp:.1f}%", {"stop_loss_pct": slp})
        if r:
            sl_results.append(r)
            print(f"  → SL={slp:.1f}%: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, TP={r['tp']}, SL={r['sl']}")
    all_results.extend(sl_results)

    # =========================================================================
    # ROUND 5: sustain_bars
    # =========================================================================
    print("\n[6/9] Sweeping sustain_bars...")
    sus_results = []
    for sb in [3, 4, 5, 6]:
        r = await run_one(f"sustain_bars={sb}", {"sustain_bars": sb})
        if r:
            sus_results.append(r)
            print(f"  → sustain={sb}: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, Sig={r['signals']}")
    all_results.extend(sus_results)

    # =========================================================================
    # ROUND 6: exhaustion_gain_pct
    # =========================================================================
    print("\n[7/9] Sweeping exhaustion_gain_pct...")
    exh_results = []
    for eg in [0.0, 5.0, 8.0, 10.0]:
        r = await run_one(f"exhaust={eg:.0f}%", {"exhaustion_gain_pct": eg})
        if r:
            exh_results.append(r)
            print(f"  → exhaust={eg:.0f}%: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, Sig={r['signals']}")
    all_results.extend(exh_results)

    # =========================================================================
    # ROUND 7: dump_volume_mult
    # =========================================================================
    print("\n[8/9] Sweeping dump_volume_mult...")
    dump_results = []
    for dv in [0.0, 2.0, 3.0, 5.0, 8.0]:
        r = await run_one(f"dump_mult={dv:.0f}", {"dump_volume_mult": dv})
        if r:
            dump_results.append(r)
            print(f"  → dump={dv:.0f}: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, Sig={r['signals']}")
    all_results.extend(dump_results)

    # =========================================================================
    # ROUND 8: partial_close_pct
    # =========================================================================
    print("\n[9/9] Sweeping partial_close_pct...")
    pc_results = []
    for pc in [40.0, 50.0, 60.0]:
        r = await run_one(f"partial={pc:.0f}%", {"partial_close_pct": pc})
        if r:
            pc_results.append(r)
            print(f"  → partial={pc:.0f}%: {r['trades']} trades, {r['win_rate']}% WR, PnL=${r['total_pnl']:+.2f}, Part={r['partials']}")
    ...
    print("\n" + "=" * 80)
    print(f"  ALL RESULTS ({len(all_results)} runs, {total_time/60:.0f} min)")
    print("=" * 80)
    print_table(baseline and [baseline] or [], "BASELINE")
    print_table(rr_results, "RISK_REWARD_RATIO sweep")
    print_table(vol_results, "VOLUME_SURGE_MULT sweep")
    print_table(cool_results, "COOLDOWN_HOURS sweep")
    print_table(sl_results, "STOP_LOSS_PCT sweep")
    print_table(sus_results, "SUSTAIN_BARS sweep")
    print_table(exh_results, "EXHAUSTION_GAIN_PCT sweep")
    print_table(dump_results, "DUMP_VOLUME_MULT sweep")
    print_table(pc_results, "PARTIAL_CLOSE_PCT sweep")

    # Find best config
    best = max(all_results, key=lambda r: r["total_pnl"])
    print(f"\n  🏆 BEST: {best['label']} — {best['trades']} trades, {best['win_rate']}% WR, PnL=${best['total_pnl']:+.2f}")
    ...
    Path("data/backtest_sweep_results.json").write_text(json.dumps(out, indent=2, default=str))
    print(f"\n  Results saved to data/backtest_sweep_results.json")

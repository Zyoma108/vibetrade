...
class CollectorsConfig(BaseModel):
    ...
    target_cycle_seconds: int = Field(
        default=0, ge=0,
        description="Целевой КАДАНС скана, с (0 = выкл, прежнее поведение). Включён — пауза "
        "после цикла считается как target минус фактическая работа, то есть каданс держится "
        "постоянным независимо от того, сколько заняла работа. Выключен — пауза фиксирована "
        "(interval_seconds), и каданс плавает вместе с длительностью цикла. "
        "Зачем: правило 4 AGENTS.md объявляет каданс частью стратегии, но код реализует "
        "фиксированный ПРОМЕЖУТОК, а не каданс. Замер боевой БД 27.08-22.09.2026: медиана "
        "кадансa 91 с при разбросе 65 с (01.09) - 141 с (29.08), то есть 2.0 скана на "
        "3-минутный бар, а по p90 всего 1.3; 1.5% циклов длиннее бара — такие бары не "
        "сканируются ни разу. Нижняя граница паузы MIN_PAUSE_SEC, чтобы быстрый цикл не "
        "превратился в горячий опрос биржи"
    )
...
class StrategyConfig(BaseModel):
    ...
    oi_trend_window_bars: int = Field(
        default=0,
        description="Окно OI-тренда в барах таймфрейма; 0 = прежнее поведение (3 последние "
        "точки). Три точки — это ДВА КАДАНСА скана, а не отрезок времени: строка OI пишется "
        "раз в цикл сбора. Замер 23.09.2026 (медиана размаха трёх точек на момент сигнала): "
        "557 с на БД 10-25.08, 187 с на 27.08-22.09, 139 с на свежей — окно сжалось вчетверо "
        "от перф-фиксов цикла, хотя стратегию никто не менял. Вместе с ним уехал и смысл "
        "oi_slope_min_pct: «+2% за окно» — это +2% за 9 минут или за 2 минуты. Значение 4 = "
        "sustain-окно детектора (4 бара x 3 мин = 12 мин), как и задумывалось изначально. "
        "Побочное следствие: при временном окне гейт перестаёт зависеть от частоты записи OI, "
        "то есть прореживание записи ради размера БД становится возможным."
    )
    ...
    undersized_verdict_min_bar_maturity_pct: float = Field(
        default=0.0, ge=0.0, le=100.0,
        description="Зрелость последнего бара (% таймфрейма), начиная с которой ДОВЕРЯЕМ "
        "вердикту «свеча слишком маленькая» (volume_fading / volume_declining) и не делаем "
        "shift-ретрай. Ниже неё вердикт вынесен по недособранным данным и сдвиг разрешён. "
        "0 = доверять всегда (поведение до 22.09.2026). Вердикты «свеча слишком большая» "
        "(volume_spike / volume_dump) блокируют сдвиг при любой зрелости — неполный бар может "
        "только вырасти. Бэктест не затрагивается: там бары закрыты, зрелость 100%. "
        "Замер 22.09.2026: 8 из 13 невидимых боту сигналов зарезаны этими двумя проверками, "
        "но грубое расширение ретрая (без условия на зрелость) дало в бэктесте -2.30R"
    )

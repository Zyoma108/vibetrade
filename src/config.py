...
class StrategyConfig(BaseModel):
    ...
    shift_retry_on_undersized_bar: bool = Field(
        default=False,
        description="Разрешить shift-ретрай, когда объёмный гейт отказал вердиктом «свеча "
        "слишком маленькая» (volume_fading / volume_declining). Последняя свеча окна в проде "
        "формирующаяся, её объём неполон ПО ПОСТРОЕНИЮ, поэтому такой отказ выносится по "
        "недособранным данным. Вердикты «свеча слишком большая» (volume_spike / volume_dump) "
        "сдвиг блокируют в любом случае — неполная свеча может только вырасти. "
